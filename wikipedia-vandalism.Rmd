...
```
In ROC space, we can appreciate mostly the same ranking as in PR space:
...
In this comparison, we can see that our solution performs very similarly as the best from @Potthast-2010. In PR Curve space, we perform slightly better on recall values between 0.4 and 0.75, but otherwise their solution has the advantage.
...
When we compare our best classifier with the best from @Potthast-2010, we find virtually equivalent PR-AUC performance. When we compare it against @Wiki-User-ClueBotNG, Wikipedia's current production classifier, we find our solution has a ~5% advantage at a FP-rate of 0.01%.

Although our solutions are head to head with both research and production systems, we do not believe a 5% gain justifies the engineering cost required to deploy such a new classifier to production. We focused on maximizing the PR-AUC metric for our work to be comparable to @Potthast-2010. We find that this metric has strong research merit, but it doesn't focus on the core Wikipedia problem: they need a sufficiently low FP-rate to be able to justify a vandalism bot that doesn't deter bona fide contributions. In future work, we will focus on improving the $TP-rate$ with the constraint that the $FP-rate <= 0.01%$.

It is reasonable to expect that a production solution for Wikipedia vandalism should strive to be easily interpretable. A user that makes a contribution that is classified as `vandalism` should understand why that is the case. We believe a comprehensive report like ours helps with this interpretabilty issue. However, we would like to explore if simpler, self-describing algorithms such as Decision Trees could achieve reasonable FP-rates while being less opaque for lay users.

Additionally, it has been argued that the use of features such as whether a user is anonymous or not can be an inequitable treatment of contributors @Laat-2015. It would be interesting then to explore in future work how to develop anti-vandalism systems that take this critique into account.
