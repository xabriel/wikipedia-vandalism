...
```{r potthast-comparison, cache=TRUE}
...
```{r cluebot-comparison, cache=TRUE, warning=FALSE}
...
In this report we have discussed the problem of detecting vandalism in Wikipedia, an online encyclopedia that anyone can edit. We utilize the PAN Wikipedia 2010 Vandalism corpus to derive a set of 24 features. We discuss, train and validate a set of binary classification algorithms. We find that Random Forests yield the best performance both from a PR-AUC, as well as a ROC-AUC perspective. Specifically, a Random Forest with 500 trees and ... yield the best performance.

When we compare our best classifier with the best from @Potthast-2010, we find virtually equivalent PR-AUC performance. When we compare it against @Wiki-User-ClueBotNG, Wikipedia's current production classifier, we find our solution has a 5% advantage at a FP-rate of 0.01%. Although our solutions are head to head with both, we do not believe a 5% gain justifies the engineering cost required to deploy such a solution to production.

In this work we focused on maximizing the PR-AUC metric for our work to be comparable to @Potthast-2010. We find that this metric has strong research merit, but it doesn't focus on the core Wikipedia problem: they need a sufficiently low FP-rate to be able to justify a deployment into production that doesn't deter bona fide contributions. In future work, we will focus on improving the $TP-rate$ with the constraint that the $FP-rate <= 0.01%$.

Additionally, a production solution for Wikipedia vandalism should strive to be easily interpretable. A user that makes a contribution that is classified as `vandalism` should understand why that is the case. We would like to explore if simpler, self-describing algorithms such as decision trees could achieve reasonable FP-rates while being less opaque for lay users. Likewise, the use of features such as whether a user is anonymous or not can be interpreted as a inequitable treatment of contributors @Laat-2015. It would be interesting then to explore how to develop anti-vandalism systems that take this critique into account.
